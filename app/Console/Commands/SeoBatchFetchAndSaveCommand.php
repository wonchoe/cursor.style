...
class SeoBatchFetchAndSaveCommand extends Command
{
    protected $signature = 'custom:seoBatchFetchSave';
    protected $description = 'Fetches results for OpenAI batch jobs, розбирає і апдейтить SeoCursorText';
    ...
    {
        Log::channel('seojobs')->info('Запущено SeoBatchFetchAndSaveCommand', ['time' => now()]);
        ...
        foreach ($pendingBatches as $batchId) {
            ...
            if ($status === 'failed') {
                ...
                $this->warn("Batch {$batchId} failed. All its items reset to new.");
                continue;
            }
            ...
            foreach ($lines as $line) {
                ...
                if (!$content) {
                    ...
                    Log::channel('seojobs')->info('Error SeoBatchFetchAndSaveCommand', ['time' => now(), 'line' => $line]);
            ...
            $this->info("Batch {$batchId}: Оновлено ".count($seoUpdates)." записів, помилок: ".count($errorLines));
        }
    }
}
